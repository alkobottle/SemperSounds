...
/// </remarks>
public sealed class EntrySoundAdmin(SoundboardDbContext db, IGuildPermissions permissions, TimeProvider? timeProvider = null)
{
    ...
    public Task<EntrySoundResult> SetEnabledAsync(ulong actorId, bool enabled, CancellationToken cancellationToken = default) =>
    ...
    public Task<EntrySoundResult> SnoozeAsync(ulong actorId, TimeSpan duration, CancellationToken cancellationToken = default)
    {
        ...
        return UpdateSettingsAsync(actorId, settings => settings.SnoozedUntil = _time.GetUtcNow() + duration, cancellationToken);
    }

    public Task<EntrySoundResult> ResumeAsync(ulong actorId, CancellationToken cancellationToken = default) =>
        UpdateSettingsAsync(actorId, settings => settings.SnoozedUntil = null, cancellationToken);

    public Task<EntrySoundResult> SetVolumeAsync(ulong actorId, int percent, CancellationToken cancellationToken = default)
    {
        if (percent is < 0 or > MaxVolumePercent)
        {
            return Task.FromResult(EntrySoundResult.Fail($"Volume has to be between 0 and {MaxVolumePercent}%."));
        }

        return UpdateSettingsAsync(actorId, settings => settings.VolumePercent = percent, cancellationToken);
    }

    public Task<EntrySoundResult> SetCooldownAsync(ulong actorId, int seconds, CancellationToken cancellationToken = default)
    {
        ...
        return UpdateSettingsAsync(actorId, settings => settings.PerUserCooldownSeconds = seconds, cancellationToken);
    }
    ...
    /// </summary>
    public Task<EntrySoundResult> SetMaxDurationAsync(ulong actorId, int milliseconds, CancellationToken cancellationToken = default)
    {
        ...
        return UpdateSettingsAsync(actorId, settings => settings.MaxDurationMs = milliseconds, cancellationToken);
    }
    ...
    public async Task<EntrySoundResult> UnblockAsync(ulong actorId, ulong targetUserId, CancellationToken cancellationToken = default)
    {
        if (Refuse(actorId) is { } refusal)
        {
            return refusal;
    ...
    private async Task<EntrySoundResult> UpdateSettingsAsync(ulong actorId, Action<EntrySoundSettings> change, CancellationToken cancellationToken)
    {
        if (Refuse(actorId) is { } refusal)
        {
            return refusal;
